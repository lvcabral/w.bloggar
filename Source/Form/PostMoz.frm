   ...
   Begin ActiveBar2LibraryCtl.ActiveBar2 acbMain 
      ...
      Begin VB.PictureBox picPost 
         ...
         Begin SizerOneLibCtl.TabOne tabPost 
            ...
            CaptionEmpty    =   ""
            Separators      =   0   'False
            Begin MOZILLACONTROLLibCtl.MozillaBrowser WebPreview 
               Height          =   3030
               Left            =   9570
               OleObjectBlob   =   "PostMoz.frx":2A34C
               TabIndex        =   7
               Top             =   375
               Width           =   8355
            End
...
On Error GoTo ErrorHandler
    ...
    If gAccount.GetPostsMethod = API_MT Or _
       gAccount.MoreTextTag1 <> "" Or _
       gAccount.MoreTextTag2 <> "" Then
...
Static InTemplateMode As Boolean
    ...
    If bolMode Then
        txtPostTit.Text = ""
        If cboPostCat.Visible Then cboPostCat.ListIndex = 0
...
On Error Resume Next
    If gSettings.DefaultBrowser Then
        Call ShellExecute(Me.hwnd, "open", gBlogs(intBlog).URL, vbNullString, CurDir$, SW_SHOW)
    Else
        WebPreview.Navigate gBlogs(intBlog).URL, 5
    End If
End Sub
...
On Error Resume Next
    If gSettings.DefaultBrowser Then
        Call ShellExecute(Me.hwnd, "open", ReadINI("CMS-" & Format(gAccount.CMS, "00"), "URL", App.Path & "\CMS\CMS.ini"), vbNullString, CurDir$, SW_SHOW)
    Else
        WebPreview.Navigate ReadINI("CMS-" & Format(gAccount.CMS, "00"), "URL", App.Path & "\CMS\CMS.ini"), 5
    End If
End Sub
...
On Error Resume Next
Dim intAux As Integer, intLin As Integer
    If ActiveControl Is Nothing Then Exit Sub
    Select Case ActiveControl.Name
...
'            ActiveControl.SelRTF = Rtf2Html(Clipboard.GetText(vbCFRTF), "+CR")
'        Else
            ActiveControl.SelText = Clipboard.GetText(vbCFText)
'        End If
        intLin = ActiveControl.GetLineFirstCharIndex
        If gSettings.ColorizeCode Then ActiveControl.Colorize IIf(intLin > intAux, intAux, intLin), intAux + Len(Clipboard.GetText) + 1
        trapMove = True
...
Dim objTool As ActiveBar2LibraryCtl.Tool, strMenu As String
    ...
    Case "bndPopTools"
        ...
        End If
    Case "bndPopPosts"
        strMenu = GetSetting(REGISTRY_KEY, "Settings", "PostMenu", "5")
...
On Error Resume Next
    ...
    ElseIf Me.Visible Then
        'Erase the PostID to avoid wrong Blog
    ...
    acbMain.RecalcLayout
    Call SaveSetting(REGISTRY_KEY, "Accounts\" & Format(gAccount.Current, "00"), "Blog", Format(Tool.CBListIndex))
    LoadBlogSettings
    LoadCategories Not Me.Visible
    'More Tab
    If gAccount.PostMethod = API_MT Or _
       gAccount.MoreTextTag1 <> "" Or _
       gAccount.MoreTextTag2 <> "" Or _
       gAccount.CMS = 32 Then
    ...
    ElseIf gAccount.CMS = 32 Then ' Just for testing purposes
        cmdMore.Visible = True
        pnlMore.Grid(gsRowHeight, 2) = 975
...
Dim strExcerpt As String, strKeywords As String
    ...
    Case "miTSpelling"
        ...
        Call CheckSpelling
    Case "miUpload"
        If Trim(gBlog.FTPHost) <> "" Then
            frmUpload.Show vbModal, Me
            If frmUpload.HtmlTag <> "" Then InsertTag frmUpload.HtmlTag, ""
            Unload frmUpload
        ...
        Else
            If MsgBox(GetMsg(msgFTPSettings), vbInformation + vbYesNo) = vbYes Then
                ...
                If Trim(gBlog.FTPHost) <> "" Then frmUpload.Show vbModal, Me
            End If
        End If
    Case "miCustomEdit"
    ...
    Case "miSettings"
        ...
        If gSettings.AppLCID <> gLCID Then
            MsgBox "You must restart w.bloggar to complete the Language change.", vbInformation
        End If
    ...
    Case "miPosts"
        If Not CanContinue() Then Exit Sub
        Select Case GetSetting(REGISTRY_KEY, "Settings", "PostMenu", "5")
        Case "0":  GetPost
    ...
    Case "miLast1"
        ...
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "1")
    Case "miLast5"
        If Not CanContinue() Then Exit Sub
        frmRecent.ShowForm 5
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "5")
    Case "miLast10"
        If Not CanContinue() Then Exit Sub
        frmRecent.ShowForm 10
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "10")
    Case "miLast15"
        If Not CanContinue() Then Exit Sub
        frmRecent.ShowForm 15
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "15")
    Case "miLast20"
        If Not CanContinue() Then Exit Sub
        frmRecent.ShowForm 20
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "20")
    Case "miLastn"
        On Error Resume Next
        If Not CanContinue() Then Exit Sub
        intPosts = Val(InputBox(GetMsg(msgHowManyPosts)))
        If intPosts > 0 Then frmRecent.ShowForm intPosts
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "n")
    Case "miPostID"
        If Not CanContinue() Then Exit Sub
        GetPost
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostMenu", "0")
...
On Error Resume Next
    ...
    SaveSetting REGISTRY_KEY, "Forms", "txtCommand.hWnd", Format(txtCommand.hwnd)
    'Load tray icon
    If Not gIsXP Then acfPost.SetTrayIcon imgIcon16.Picture
End Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    ...
    If Not Cancel Then
        ...
        Else
            gSettings.PostFile = ""
        End If
        Call SaveSetting(REGISTRY_KEY, "Settings", "PostFile", gSettings.PostFile)
    End If
End Sub
...
Dim strPreview As String, strTemp As String
    strPost = txtPost.Text
    ...
    Else
        ...
        strMore = GetMore(strPost) & txtMore.Text
        ...
        strPreview = "<html><head><title>Bloggar Preview</title></head>" & vbCrLf & _
                     gBlog.PreviewCSS & vbCrLf & gBlog.PreviewBody & vbCrLf & _
...
Dim strPost As String, strTitle As String, strFName As String
    ...
    If FileExists(strFile) Then
        ...
        Else 'New xml .post format
            ...
            If PostData.AccountID >= 0 And PostData.AccountID <> gAccount.Current Then
                MsgBox "This Post file does not belongs to the current account.", vbExclamation
                Exit Sub
            ElseIf PostData.BlogID <> "" And PostData.BlogID <> gBlogs(acbMain.Bands("bndTools").Tools("miBlogs").CBListIndex).BlogID Then
                MsgBox "This Post file does not belongs to the current blog.", vbExclamation
                Exit Sub
            Else
        ...
        With cboPostCat
            If .ListIndex > 0 Then
                If Len(PostData.Categories) > 0 Then
                    PostData.Categories = Mid(PostData.Categories, 6)
                End If
                If InStr(PostData.Categories, Format(.ItemData(.ListIndex), "0000")) > 0 Then
                    PostData.Categories = Replace(PostData.Categories, Format(.ItemData(.ListIndex), "0000") & vbTab, "")
                End If
                PostData.Categories = Format(.ItemData(.ListIndex), "0000") & vbTab & PostData.Categories
            Else
                PostData.Categories = ""
            End If
        End With
